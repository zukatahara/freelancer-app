export const jobCates = [
  {
    icon: '/icons/cate-software.svg',
    title: 'Trang web, IT & Phần mềm',
    description: 'PHP, HTML, JavaScript, Kiến trúc phần mềm, WordPress...',
    jobs: [
      {
        name: '.NET',
        quantity: 74,
        href: '/dot-net',
      },
      {
        name: '.NET Core Web API',
        quantity: 4,
        href: '/dot-net-core',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'Advanced Bussiness Application Programming (ABAP)',
        quantity: 15,
        href: '/advanced-bussiness-application',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
    ],
  },
  {
    icon: '/icons/cate-mobile-pc.svg',
    title: 'Điện thoại di động & Máy tính',
    description: 'Mobile App Development, Android, iPhone, iPad, Kotlin...',
    jobs: [
      {
        name: '.NET',
        quantity: 74,
        href: '/dot-net',
      },
      {
        name: '.NET Core Web API',
        quantity: 4,
        href: '/dot-net-core',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'Advanced Bussiness Application Programming (ABAP)',
        quantity: 15,
        href: '/advanced-bussiness-application',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
    ],
  },
  {
    icon: '/icons/cate-content.svg',
    title: 'Sáng tác & Nội dung',
    description:
      'Soạn thảo nội dung, Article Writing, Viết quảng cáo, Ghostwriting, Dịch thuật....',
    jobs: [
      {
        name: '.NET',
        quantity: 74,
        href: '/dot-net',
      },
      {
        name: '.NET Core Web API',
        quantity: 4,
        href: '/dot-net-core',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'Advanced Bussiness Application Programming (ABAP)',
        quantity: 15,
        href: '/advanced-bussiness-application',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
    ],
  },
  {
    icon: '/icons/cate-telecom.svg',
    title: 'Thiết kế, Truyền thông & Kiến trúc',
    description:
      'Thiết kế đồ họa, Thiết kế trang web, Photoshop, Thiết kế logo, Illustrator...',
    jobs: [
      {
        name: '.NET',
        quantity: 74,
        href: '/dot-net',
      },
      {
        name: '.NET Core Web API',
        quantity: 4,
        href: '/dot-net-core',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'Advanced Bussiness Application Programming (ABAP)',
        quantity: 15,
        href: '/advanced-bussiness-application',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
    ],
  },
  {
    icon: '/icons/cate-keyboard.svg',
    title: 'Nhập liệu và Hành chính',
    description:
      'Nhập liệu, Excel, Xử lí dữ liệu, Tìm kiếm web, Hỗ trợ khách hàng...',
    jobs: [
      {
        name: '.NET',
        quantity: 74,
        href: '/dot-net',
      },
      {
        name: '.NET Core Web API',
        quantity: 4,
        href: '/dot-net-core',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'Advanced Bussiness Application Programming (ABAP)',
        quantity: 15,
        href: '/advanced-bussiness-application',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
    ],
  },
  {
    icon: '/icons/cate-science.svg',
    title: 'Khoa học & Kĩ thuật',
    description: 'AutoCAD, Kĩ thuật, CAD/CAM, Kĩ thuật điện, Điện tử...',
    jobs: [
      {
        name: '.NET',
        quantity: 74,
        href: '/dot-net',
      },
      {
        name: '.NET Core Web API',
        quantity: 4,
        href: '/dot-net-core',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'Advanced Bussiness Application Programming (ABAP)',
        quantity: 15,
        href: '/advanced-bussiness-application',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
    ],
  },
  {
    icon: '/icons/cate-manufacture.svg',
    title: 'Tìm nguồn sản phẩm & Sản xuất',
    description:
      'Thiết kế sản phẩm, Product Research, Process Automation, Tìm nguồn cung ứng sản phẩm, Tìm nguồn cung ứng...',
    jobs: [
      {
        name: '.NET',
        quantity: 74,
        href: '/dot-net',
      },
      {
        name: '.NET Core Web API',
        quantity: 4,
        href: '/dot-net-core',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'Advanced Bussiness Application Programming (ABAP)',
        quantity: 15,
        href: '/advanced-bussiness-application',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
    ],
  },
  {
    icon: '/icons/cate-marketing.svg',
    title: 'Tiếp thị & Bán hàng',
    description:
      'Tiếp thị qua Internet, Tiếp thị, Quảng bá truyền thông, Quảng cáo trên Facebook, Bán hàng...',
    jobs: [
      {
        name: '.NET',
        quantity: 74,
        href: '/dot-net',
      },
      {
        name: '.NET Core Web API',
        quantity: 4,
        href: '/dot-net-core',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'Advanced Bussiness Application Programming (ABAP)',
        quantity: 15,
        href: '/advanced-bussiness-application',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
      {
        name: 'A+ Certified Professional',
        quantity: 1,
        href: '/a-certified',
      },
    ],
  },
];
